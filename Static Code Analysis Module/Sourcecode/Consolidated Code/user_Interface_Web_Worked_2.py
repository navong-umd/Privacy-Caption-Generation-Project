...
PROG_ORDER = [
    ...
    "disclosure_ads", "disclosure_affiliates", "disclosure_partners", "disclosure_security",
...
<head>
  ...
  <script>
    ...
    document.addEventListener('DOMContentLoaded', function(){
      ...
      // Existing toggles
      bindToggle('inc_yes','desc_block'); // handled separately earlier but safe
    ...
    <div class="group">
      ...
      <div class="group">
        ...
        <div style="display:flex;gap:10px;align-items:center">
          <input id="own_text" name="own_text" type="text" style="flex:1" value="">
          <button id="add_item_btn" class="btn-link" type="button">+ ADD</button>
        </div>
      </div>
    </div>
  </div>
  ...
  <div class="group">
    ...
    <div class="group">
      ...
      <div style="display:flex;gap:10px;align-items:center">
        ...
        <button id="pi_add" class="btn-link" type="button">+ ADD</button>
      </div>
    </div>
  </div>
...
<div class="group">
    <div class="qrow">
      ...
      <span class="qhelp" aria-live="polite">
        ...
        <div class="popover" role="dialog" aria-modal="false">
          <h4>Note:</h4>
          <p>A third-party advertiser is an external company or platform, such as Google AdMob, Facebook Audience Network, or AppLovin, that uses personal information collected from mobile apps (like device identifiers and usage data) to serve targeted advertisements to users within those apps.</p>
        </div>
      </span>
    </div>
    <form method="post" action="{{ url_for('disclosure_ads') }}">
      <div class="group">
        <label><input type="radio" id="ua_yes" name="ads" value="yes" {{ 'checked' if ads=='yes' else '' }}> Yes</label>
        <label><input type="radio" id="ua_no"  name="ads" value="no"  {{ 'checked' if ads!='yes' else '' }}> No</label>
      </div>
      <div class="actions">
...
<div class="group">
    <div class="qrow">
      ...
      <span class="qhelp" aria-live="polite">
        ...
        <div class="popover" role="dialog" aria-modal="false">
          <h4>Note:</h4>
          <p>Business affiliates include your parent company, subsidiaries, and joint venture partners.</p>
        </div>
      </span>
    </div>
    <form method="post" action="{{ url_for('disclosure_affiliates') }}">
      <div class="group">
        <label><input type="radio" id="ua_yes" name="aff" value="yes" {{ 'checked' if aff=='yes' else '' }}> Yes</label>
        <label><input type="radio" id="ua_no"  name="aff" value="no"  {{ 'checked' if aff!='yes' else '' }}> No</label>
      </div>
      <div class="actions">
...
<div class="group">
    <div class="qrow">
      ...
      <span class="qhelp" aria-live="polite">
        ...
        <div class="popover" role="dialog" aria-modal="false">
          <h4>Note:</h4>
          <p>A business partner means a person or company that works together with your company sharing responsibilities, risks, or resources to help achieve common business objectives.</p>
        </div>
      </span>
    </div>
    <form method="post" action="{{ url_for('disclosure_partners') }}">
      <div class="group">
        <label><input type="radio" id="ua_yes" name="partners" value="yes" {{ 'checked' if partners=='yes' else '' }}> Yes</label>
        <label><input type="radio" id="ua_no"  name="partners" value="no"  {{ 'checked' if partners!='yes' else '' }}> No</label>
      </div>
      <div class="actions">
        <a class="btn btn-ghost" href="{{ url_for('disclosure_affiliates') }}">Back</a>
        <button class="btn btn-accent" type="submit" name="next" value="1">Next</button>
      </div>
  </form>
</div>
...
<form method="post" action="{{ url_for('disclosure_security') }}">
  ...
  <div class="actions">
    <a class="btn btn-ghost" href="{{ url_for('disclosure_partners') }}">Back</a>
    <button class="btn btn-accent" type="submit" name="next" value="1">Next</button>
  </div>
</form>
"""
...
<form method="post" action="{{ url_for('tracking_main') }}">
  <div class="group">
    <div class="qrow"><label style="font-weight:700">Which of the following does your website or app use or plan to use?</label></div>
...
<form method="post" action="{{ url_for('user_rights_dsar') }}">
  ...
  <!-- NEW: How can users contact you? (matches page9_add1/page9_add2) -->
...
<form method="post" action="{{ url_for('final_title') }}">
  <div class="group">
    ...
    <div class="radios" style="flex-direction:column;align-items:flex-start">
      ...
      <label><input type="radio" name="ptitle_opt" value="Privacy Notice" {{ 'checked' if ptitle_opt=='Privacy Notice' else '' }}> Privacy Notice</label>
      <label><input type="radio" name="ptitle_opt" value="none" {{ 'checked' if ptitle_opt=='none' else '' }}> I don't want to include a title</label>
...
def page4():
    ...
    if request.method == "POST":
        ...
        custom_items = []
        if can_register == "yes":
            update_opts = request.form.getlist("update_opts")
            custom_items = request.form.getlist("custom_items")
            summary = ["User accounts enabled."]
            if update_opts:
                summary.append("Update/Delete options: " + ", ".join(update_opts))
            if custom_items:
                summary.append("Custom items: " + ", ".join(custom_items))
            flash(" | ".join(summary))
        else:
    ...
    return render_page(PAGE4_BODY, section_step=2, endpoint_name="page4",
...
def collection_direct():
    ...
    if request.method == "POST":
        pi_opts = request.form.getlist("pi_opts")
        pi_items = request.form.getlist("pi_items")
        device_opts = request.form.getlist("device_opts")
        ...
        if chosen_labels:
            flash("Selected (direct collection): " + ", ".join(chosen_labels))
        else:
            flash("No items selected for direct collection.")
        if dev_labels:
            flash("Selected (device collection): " + ", ".join(dev_labels))
...
def disclosure_ads():
    prev = answers().get("Disclosure - Ads", {})
    ads = prev.get("ads")
    if request.method == "POST":
        ads = request.form.get("ads","no")
        set_section("Disclosure - Ads", {"ads": ads})
        flash(f"Third-party ads: {ads}")
        return redirect(url_for('disclosure_affiliates'))
    return render_page(DISC_ADS_BODY, section_step=5, endpoint_name="disclosure_ads", ads=ads)

@app.route("/disclosure/affiliates", methods=["GET","POST"], endpoint="disclosure_affiliates")
def disclosure_affiliates():
    prev = answers().get("Disclosure - Affiliates", {})
    aff = prev.get("aff")
    if request.method == "POST":
        aff = request.form.get("aff","no")
        set_section("Disclosure - Affiliates", {"aff": aff})
        flash(f"Business affiliates: {aff}")
        return redirect(url_for('disclosure_partners'))
    return render_page(DISC_AFFILIATES_BODY, section_step=5, endpoint_name="disclosure_affiliates", aff=aff)

@app.route("/disclosure/partners", methods=["GET","POST"], endpoint="disclosure_partners")
def disclosure_partners():
    prev = answers().get("Disclosure - Partners", {})
    partners = prev.get("partners")
    if request.method == "POST":
    ...
    return render_page(DISC_PARTNERS_BODY, section_step=5, endpoint_name="disclosure_partners", partners=partners)
...
def disclosure_security():
    ...
    if request.method == "POST":
        ...
        if sec == "no" and not sec_ack:
            ...
            # Re-render so the warning is visible and the checkbox is required
...
def user_rights_dsar():
    ...
    if request.method == "POST":
        ...
        # Flash summary
        contact_bits = []
        if contact_email and contact_email_addr: contact_bits.append(f"email: {contact_email_addr}")
        if contact_web and contact_web_link: contact_bits.append(f"web: {contact_web_link}")
        if contact_phone and contact_phone_number: contact_bits.append(f"phone: {contact_phone_number}")
        if contact_mail and contact_post_addr: contact_bits.append("postal address provided")
        if dsar == "yes":
...
def final_company():
    ...
    return render_page(
        ...
        company_name=company_name, dba=dba, privacy_email=privacy_email, phone=phone
    )
    ...
    if request.method == "POST":
        if request.form.get("publish"):
            ...
            try:
                ...
                flash(f"Error saving questionnaires: {e}")
...
def dump_answers_to_excels():
    ...
    website_url = dsar_sec.get("contact_web_link","")
    ...
    MASTER_COLUMNS = [
        ...
        "collect_personal_data", "data_categories", "collection_methods", "source_third_party",
        "minors_data",
    ...
    with pd.ExcelWriter(by_path, engine="openpyxl") as xw:
        ...
        # Data Collection
        dc_cols = ["apk_saved_path","collect_personal_data","collection_methods","data_categories","source_third_party","submission_id","submitted_at"]
        dc_row = {
            "apk_saved_path": apk_saved_path,
            "collect_personal_data": collect_personal_data,
            "collection_methods": collection_methods,
